#!/usr/bin/env node
/**
 * Test script for the cron rotation endpoint.
 * 
 * Usage:
...
const BASE_PROD = "http://104.248.217.57:3000";
...
async function testDryRun() {
  ...
  // Validate distribution: each branch should have employees
  if (data.branchSummary) {
    ...
    // Check balance: no branch should have more than ceil(total/branches) + 1
...
async function testRandomness() {
  ...
  // At least 2 out of 3 runs should produce different assignments
...
async function main() {
  console.log(bold(`\n${"═".repeat(50)}`));
  console.log(bold(`  ShiftCrew Cron Rotation Tests`));

...
export default function SchedulingPage() {
  ...
  const getFirestoreEmployeesForBranch = useCallback((branchId: string | number) => {
    ...
    for (const emp of fireEmployees) {
      ...
      if (manualBranch !== undefined) {
        // Explicit manual assignment exists — respect it.
        // "__unassigned__" means user deliberately unassigned; skip entirely.
        if (manualBranch === "__unassigned__") continue
  ...
  // Prepare balanced desired totals per branch (base + distribute remainder)
    ...
    // Filter out "__unassigned__" sentinel — these employees are deliberately excluded
    const activeManual: Record<string, string> = {}
    for (const [k, v] of Object.entries(effectiveManual)) {
      if (v && v !== "__unassigned__") activeManual[k] = v
    }
    const manualAssignedIds = Object.keys(effectiveManual).map((k) => String(k)) // includes __unassigned__ so they're excluded from auto-rotation
    const manualCount: Record<string, number> = {}
    for (const idStr of Object.keys(activeManual)) {
      const bn = activeManual[idStr]
      if (!bn) continue
    ...
    // First, apply manual assignments (always honor — uses activeManual which excludes __unassigned__)
    for (const idStr of Object.keys(activeManual)) {
      const id = idStr
    ...
    try {
      ...
      if (foundSchedules && foundSchedules.length > 0) {
        ...
        if (rotationData.length > 0) {
          ...
          // Do NOT restore manualAssignments from Firestore — manualAssignments is session-only.
          // The isManual flags live in appliedRotation for display and generateRotation reads them there.
          if (latestDate) {
  ...
  const handleManualAssignment = (employeeId: string | number, branchName: string) => {
    ...
    // "__unassigned__" is never evicted — it's an explicit user intent.
    setManualAssignments((prev) => {
      const updated: Record<string, string> = {}
      for (const [empId, branch] of Object.entries(prev)) {
        if (branch === branchName && branch !== "__unassigned__" && empId !== key) continue // evict previous occupant
  ...
  const handleAssignCrewFirestore = async (crewId: string | number, branchId: string, employee: any) => {
    if (branchId) {
      ...
      if (branchId === "unassigned") {
        // Mark as explicitly unassigned (sentinel value) so the priority chain
        // does NOT fall through to appliedRotation or employee.branchId
        setManualAssignments((prev) => ({
          ...prev,
          [String(employee.id)]: "__unassigned__",
        }))
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      // Build complete assignments array from the best available source:
      // 1. Start with appliedRotation (current week's schedule) as the base for ALL employees
      // 2. Fall back to rotatedEmployees (preview) if no applied rotation exists
      // 3. Override with session-only manualAssignments
      ...
      // Determine base source: appliedRotation (schedule) > rotatedEmployees (preview)
      const baseSource = (appliedRotation && appliedRotation.length > 0) ? appliedRotation : rotatedEmployees

      if (baseSource && baseSource.length > 0) {
        for (const emp of baseSource) {
          const fireBranch = fireBranches.find((fb: any) => fb.branchName === emp.nextWeekBranch || fb.name === emp.nextWeekBranch)
      ...
      // Override with session-only manual assignments (only what the user explicitly set this session)
      for (const [employeeIdStr, branchName] of Object.entries(manualAssignments)) {
        // Skip __unassigned__ sentinel — these employees should be excluded from the saved schedule
        if (branchName === "__unassigned__") {
          // Remove from allAssignments if present (from base source)
          const idx = allAssignments.findIndex((a) => String(a.employeeId) === employeeIdStr)
